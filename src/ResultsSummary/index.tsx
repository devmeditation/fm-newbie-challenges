    ...
    <section className={styles.resultsSummary}>
      ...
      <div className={styles.cardContainer}>
        ...
        {DATABASE_CARD.map((card) => {
          return (
            <div key={card.color} className={styles.card} style={{background:card.backgroundColor}}>
              <div className={styles.cardIconAndTitle} style={{color: card.color}}>
                {card.icon}
                <strong> {card.title}</strong> 
              </div>
              <div>
                <span className={styles.cardResultTotal}>{card.total}</span>  / 100
              </div>
            </div>
          );
        })}

        <button className={styles.cardButton}>Continue</button>
      </div>
    </section>
  );
}
